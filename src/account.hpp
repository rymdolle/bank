...
public:
	...
	static std::vector<Account> getAccounts(int user)
	{
		std::ifstream file("data/accounts.txt");
		...
		while (std::getline(file, line))
		{
			...
			std::string accountName, amount, userId, id;
			std::getline(ss, userId, ':');
			std::getline(ss, id, ':');
			std::getline(ss, amount, ':');
			std::getline(ss, accountName, ':');

			if (std::stoi(userId) == user)
				accounts.emplace_back(accountName, std::stod(amount), std::stoi(userId), std::stoi(id));
		...
		{
			i++;
			std::cout << "\n" << (i) << ".\t" << acc.getAccountName() << "\t\t" << acc.getBalance() << "\t\t" << acc.getUserId() << "\t\t"
				<< acc.getId() << "\t\t" << "\n";
		}
